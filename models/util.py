...
def download_fromhub(path, tdir=""):
    ...
    if path.startswith("jittorhub://"):
        path = path.replace("jittorhub://", f"https://cg.cs.tsinghua.edu.cn/jittor/{tdir}assets/build/checkpoints/")
        base = path.split("/")[-1]
        fname = os.path.join(compiler.ck_path, tdir, base)
        from jittor_utils.misc import download_url_to_local
        if not os.path.exists(fname):
            download_url_to_local(path, base, os.path.join(compiler.ck_path, tdir), None)
        return fname
